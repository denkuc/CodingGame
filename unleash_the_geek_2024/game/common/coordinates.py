...
from entity.dictionaries import VECTORS
...
class Coordinates(StringifyInterface):
    ...
    def neighbors(self):
        return CoordinatesCollection([self + vector for vector in VECTORS.values()])

    def same_line(self, coordinates: 'Coordinates') -> bool:
        return self.x == coordinates.x or self.y == coordinates.y

    def get_coordinates_between(self, coordinates: 'Coordinates') -> 'CoordinatesCollection':
        coordinates_between = CoordinatesCollection()
        if self.same_line(coordinates):
            if self.x == coordinates.x:
                for i in range(min(self.y, coordinates.y) + 1, max(self.y, coordinates.y)):
                    coordinates_between.add(Coordinates(self.x, i))
            else:
                for i in range(min(self.x, coordinates.x) + 1, max(self.x, coordinates.x)):
                    coordinates_between.add(Coordinates(i, self.y))
